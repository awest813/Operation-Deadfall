...
void M_Menu_Lighting_f (void)
{
...

	{
		...
		menubulk_t bulk[] =
		{
			...
			MB_COMBOCVAR("Powerup Glow", r_powerupglow, powerupopts, powerupvalues, "Dynamic light on players and items with power-up effects. Non-self hides the glow on your own view model only (others still see it)."),
			MB_CHECKBOXCVARTIP("Powerup Shell", v_powerupshell, 0, "Shell tint on models with Quad (blue), Pent (yellow), or Ring (grey). Intensity scales with the cvar."),
			MB_SPACING(4),
